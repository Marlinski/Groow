"""Arithmetic: `arithmetic drill [--rounds N] [--level 1|2|3]` mental arithmetic whose right and wrong answers become training samples."""
import json
import os
import random
import re
...
def _drill(rounds=3, n=16, level=2, rng=None):
    ...
    for r in range(rounds):
        ...
        with (tdir / "arithmetic.jsonl").open("a") as f:
            for (q, ans), conv, out in zip(qs, convs, outs):
                ...
                correct += reward == 1.0
                f.write(json.dumps({"ts": time.time(), "kind": "pg", "prompt": conv, "completion": (out or "").strip()[:20],
                                    "reward": reward, "group": group, "source": f"arithmetic level {level}"}) + "\n")
        summary.append({"round": r + 1, "accuracy": round(correct / n, 3)})
    return {"rounds": summary, "training_set": str(tdir / "arithmetic.jsonl"),
            "note": "samples are pending until the next nap or night"}


def main(argv: list) -> dict:
    """arithmetic drill [--rounds N] [--level 1|2|3]"""
    if not argv or argv[0] in ("-h", "--help"):
        return {"text": "usage: arithmetic drill [--rounds 3] [--level 2]   questions, answers scored -> training samples"}
    if argv[0] == "--selftest":
